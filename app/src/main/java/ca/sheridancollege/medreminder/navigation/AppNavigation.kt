...
import ca.sheridancollege.medreminder.presentation.add.AddMedicationViewModel
    ...
    Scaffold(
        ...
        NavHost(navController, startDestination = Screen.Today.route, modifier = Modifier.padding(innerPadding)) {
            ...
            composable(
                ...
                arguments = listOf(
                    androidx.navigation.navArgument("medicationId") {
                        type = androidx.navigation.NavType.IntType
                    }
                )
            ) { backStackEntry ->
                val id = backStackEntry.arguments?.getInt("medicationId") ?: 0
                val vm: AddMedicationViewModel = hiltViewModel()
                LaunchedEffect(id) { vm.loadMedication(id) }
                AddMedicationScreen(
